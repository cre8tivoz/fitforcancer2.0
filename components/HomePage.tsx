...
const HomePage: React.FC<HomePageProps> = ({ fatigueZone, onNavigate }) => {
  const zoneKey = fatigueZone?.split(' ')[1] as 'Green' | 'Yellow' | 'Red' | undefined;
  const featuredMovement = zoneKey
    ? (MOVEMENTS.find((movement) => movement.intensity === zoneKey) ?? MOVEMENTS[0])
    : MOVEMENTS[0];
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="flex flex-col gap-3 p-4 bg-slate-50 rounded-xl border border-slate-100">
              ...
              <div>
                <h3 className="font-bold text-slate-900 text-sm mb-1">Check your fatigue</h3>
                <p className="text-xs text-slate-600 leading-relaxed">Pick 0–10 for how fatigued you feel today: 0 means no fatigue, 10 means the worst fatigue.</p>
              </div>
            </div>
      ...
      <section>
        ...
        <MovementCard movement={featuredMovement} />
      </section>
    </div>
  );
};
